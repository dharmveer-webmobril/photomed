...
export default function SubscriptionManage({ props }) {
  ...
  // const needsSubscription = token && accessToken && (!currentSubscription?.hascurrentSubscription || !currentSubscription?.isActive);
  ...
  const getMySubscriptionDetails = async () => {
    ...
    try {
      const response = await fetch(
        "http://photomedpro.com/api/check-user-subscription",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok && responseData?.transactionId) {
        console.log("getMySubscriptionDetails response:", responseData);
        setCurrentSubscription(responseData);
      } else {
  ...
  const restorePurchases = async () => {
    ...
    try {
      ...
      if (purchases.length > 0) {
        ...
        const response = await fetch(
          "http://photomedpro.com/api/verify-inapp-receipt",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
