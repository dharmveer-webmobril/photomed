...
const ConnectCloud = () => {
  ...
  const connectDrive = async () => {
    ...
    try {
      ...
      const tokens = await exchangeGoogleCodeForTokens(userInfo.data.serverAuthCode);
      if (tokens?.access_token) {
        const { access_token, refresh_token, expires_in } = tokens;
        const expiry = Date.now() + expires_in * 1000;
        dispatch(setAccessToken(access_token));
        await storeData('refresh_token', refresh_token);
        await storeData('token_expiry', expiry.toString());
        dispatch(setCloudType('google'));
        Toast.show('Google Drive Connected Successfully');
      }
    } catch (error) {
