...
export default function DermoScopyImageCompare() {
  ...
  const imagesParam = useMemo(
    () => route?.params?.images || [],
    [route?.params?.images],
  );
  const onSave = route?.params?.onSave;
  const uploadToCloud = route?.params?.uploadToCloud;
  const saveMainImage = route?.params?.saveMainImage;
  const body = route?.params?.body;
  const circleName = route?.params?.circleName;
  const isCaptureMode = !!onSave;
  const isMainImageCapture = isCaptureMode && !circleName;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Save failed",
        err?.message || "Could not save images to cloud. Please try again.",
      );
    } finally {
      setIsSaving(false);
    }
          ...
          <>
            ...
            <View style={styles.controlsRow}>
              ...
              <TouchableOpacity
                ...
                style={[
                  styles.saveBtn,
                  (capturedImages.length === 0 || isSaving) &&
            ...
            {capturedImages.length > 0 && (
              <View style={styles.imageListContainer}>
                ...
                {isMainImageCapture && (
                  <Text style={styles.newPhotosLabel}>
                    Captured Image (tap to retake):
                  </Text>
                )}
                <FlatList
                  horizontal
